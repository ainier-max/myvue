  ...
  <div style="height: 100%; overflow: hidden; margin: 0px; background: #181b1e">
    ...
    >
      <div align="right" id="operateMenuID" style="cursor: move; padding: 10px 10px 10px 10px; border-bottom: 1px solid gray">
        ...
        -->
        <el-button v-if="designTab == 'second' && debugProcessFlag == true" type="danger" size="small" @click="runProcess">运行 </el-button>
        <el-button v-if="designTab == 'second' && debugProcessFlag == true" type="danger" size="small" @click="resetProcess">重置调试状态 </el-button>
        <el-button v-if="designTab == 'second' && debugProcessFlag == true" type="danger" size="small" @click="stopDebugProcess"
          >关闭调试界面
        </el-button>
      </div>
      ...
      <div style="overflow: auto; position: relative; height: 850px; overflow: hidden">
        <PageDebug
          ref="pageDebugRef"
          page_debug_flag_from_parent="true"
          :page_id_from_parent="page_id"
        ...
        <div style="width: 100%; height: 100%; overflow-y: hidden">
          <div class="box left" id="leftBox">
            ...
            <div class="resize-box" id="resize-box">
              ...
              <el-button
                v-if="designTab == 'second' && debugProcessFlag == false"
                style="position: absolute; right: 130px; top: 8px"
                type="success"
                size="small"
              ...
              <el-button
                v-if="currentPageBlock.page_block_ref != null"
                style="position: absolute; right: 70px; top: 8px"
                type="primary"
                size="small"
                @click="savePageBack"
                >保存
          ...
          <div class="box right" id="rightDiv" style="background: #282c34; overflow-y: auto">
            <PageSettings v-if="designTab == 'first' && SettingsTab == 'PageSettings'" :currentPageBlock="currentPageBlock"></PageSettings>
            <PageBlockSettings
              v-if="designTab == 'first' && SettingsTab == 'PageBlockSettings'"
              :currentPageBlock="currentPageBlock"
            ></PageBlockSettings>
            <PageBlockLayoutSetting
              v-if="designTab == 'first' && SettingsTab == 'PageBlockLayoutSetting'"
              :currentPageBlockLayout="currentPageBlockLayout"
            ></PageBlockLayoutSetting>
            <PageBlockComponentSetting
              v-if="designTab == 'first' && SettingsTab == 'PageBlockComponentSetting'"
              :currentPageBlockComponent="currentPageBlockComponent"
            ></PageBlockComponentSetting>
        ...
        for (let i = 0; i < this.pageLayouts.length; i++) {
          if (this.pageLayouts[i].layout_ref == item.layout_ref) {
            ...
            for (let j = 0; j < this.pageLayouts[i].layout_config.blocks_components.length; j++) {
              if (
                this.pageLayouts[i].layout_config.blocks_components[j].type == "component" &&
                this.pageLayouts[i].layout_config.blocks_components[j].ref == item.component_ref
              ) {
                let obj = {};
