<template>
    <div style="width: 100%; height: 100%; overflow: auto">
      ...
      <div v-if="currentPageRenderTreeNodeData">
        <div class="leftTitle" style="font-weight: bolder">
            类型：
            <span v-if="currentPageRenderTreeNodeData.type == 'flex-row'">水平布局</span>
            <span v-if="currentPageRenderTreeNodeData.type == 'flex-column'">垂直布局</span>
            <span v-if="currentPageRenderTreeNodeData.type == 'frontEndComponent'">前端组件</span>
        </div>
        ...
        >
          <div class="leftTitle">
            ...
            <el-popover
              ...
              @show="showPopover"
            >
            <div class="custom-content" v-html="customContent">
  ...
  const showPopover=()=>{
    let nodeTemp= findNodeById(pageRenderTreeData.value, currentPageRenderTreeNodeData.value.pid);
    let sumFlexBasic=0;
    for(let i=0;i<nodeTemp.children.length;i++){
      console.log("nodeTemp.children[i]",nodeTemp.children[i]);
      sumFlexBasic=sumFlexBasic+parseInt(nodeTemp.children[i].config.attr.flexBasis);
    }
    console.log("showPopover--sumFlexBasic",sumFlexBasic);
    let strTemp="";
    strTemp=strTemp+"该节点的父节点是《"+nodeTemp.label+"》，其《布局占比总和》为："+sumFlexBasic+"<br>";
    for(let i=0;i<nodeTemp.children.length;i++){
      let labelTemp=nodeTemp.children[i].label;
      let flexBasisTemp=nodeTemp.children[i].config.attr.flexBasis
      strTemp=strTemp+"布局"+(i+1)+"《"+labelTemp+"》当前占比为："+flexBasisTemp+"，比例大小为："+flexBasisTemp+"/"+sumFlexBasic+"<br>";
    }
