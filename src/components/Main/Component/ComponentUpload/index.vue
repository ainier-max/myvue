  ...
  <div style="width: 100%; height: 100%; overflow-y: hidden">
    <div class="box left" id="leftBox">
      ...
      <div class="resize-box" id="resize-box">
        ...
        >
          <el-radio-button value="vue">组件代码</el-radio-button>
          <el-radio-button value="config">开发者配置</el-radio-button>
          <el-radio-button value="visualize">可视化配置</el-radio-button>
        </el-radio-group>
        ...
        <MyMonacoEditor
          ...
          "
          :id="vueCodeID"
          :code="this.componentInfo.component_code"
          @update="updateConfig"
        ></MyMonacoEditor>
        ...
        <MyMonacoEditor
          ...
          "
          :id="configCodeID"
          :code="component_config_str"
          @update="updateConfig"
        ></MyMonacoEditor>

        <!--可视化配置-->
        <div
          v-if="changeType == 'visualize' && this.componentInfo != null"
          style="width: 100%; height: 100%; padding: 30px; background: #282c34"
        >
          <el-tabs
            v-model="activeName"
            class="demo-tabs"
            @tab-change="tabsHandleClick"
          ...
          >
            ...
            >
              <MyMonacoEditor
                ref="visualizeCodeRef"
                v-if="codeShowFlag == true"
                :id="visualizeCodeID"
                :code="component_visualize_str"
                @update="updateConfig"
                style="width: 100%; height: 100%"
              ></MyMonacoEditor>
            </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    findComponentInfoCallBack(result) {
      this.componentInfo = {};
      this.component_config_str = js_beautify(
      ...
      this.componentInfo.config = stringToObject(
        result.objects[0].component_config_str
      );
    ...
    setVisualizeOptions(component_visualize_str) {
      this.visualizeOptionsTemp = eval(
        "(" + component_visualize_str + ")"
      ).visualizeOptions;
      for (let i = 0; i < this.visualizeOptionsTemp.length; i++) {
        const strTemp =
          "this.componentInfo.config." + this.visualizeOptionsTemp[i].name;
    ...
    updateConfigParam(item) {
      ...
      if (
        ...
        strTemp =
          "this.componentInfo.config." + item.name + "='" + item.value + "'";
      } else {
    ...
    changeTypeFun(val) {
      ...
      if (val == "config") {
        this.component_config_str = js_beautify(
          objectToString(this.componentInfo.config)
        );
      }
      if (val == "visualize") {
        this.setVisualizeOptions(this.component_visualize_str);
