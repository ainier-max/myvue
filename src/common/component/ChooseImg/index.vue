  ...
  <div>
    <div v-if="imgUrl!='' && imgUrl.indexOf('rgba(')<0">
      <el-popconfirm title="确定删除该照片?"
                     ...
                     @cancel="cancelEvent">
        <template #reference>
          <img :src="imgUrl" style="width: 50px;height: 50px;padding-right: 10px">
        </template>
      </el-popconfirm>
    </div>
...
export default {
  ...
  created() {
    if(this.imgUUID!=""){
      this.imgUrl=this.getFileUrl+"?uuid=" + this.imgUUID + "&type=photo";
    }
  },
  watch: {
  ...
  methods: {
    ...
    confirmEvent() {
      this.imgUrl="";
      this.$emit("getNewImgUrl","",this.otherParam);
    },
    cancelEvent(){
      console.log("取消");
    },
    ...
    handleAvatarSuccess(res, file) {
      ...
      this.imgUrl=this.getFileUrl+"?uuid="+fileObj.uuid+"&type=photo";
      this.$emit("getNewImgUrl",fileObj.uuid,this.otherParam);
    },
    //获取url参数
